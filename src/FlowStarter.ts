    ...
    for (const flow of this.flows) {
      ...
      combineLatest([storeObservable, ...observables])
        ...
        .subscribe((states) => {
          ...
          if (canStartFlow) {
            const onWillChooseFlowRawReturn = this.config.onWillChooseFlow(flow);

            const onWillChooseFlowReturn = ((): OnWillChooseFlowReturn => {
              if (typeof onWillChooseFlowRawReturn == 'object' && onWillChooseFlowRawReturn != null) {
                if ((onWillChooseFlowRawReturn as FlowObj).flowId != null) {
                  return {
                      overridingFlow: onWillChooseFlowRawReturn as FlowObj
                    };
                } else {
                  return onWillChooseFlowRawReturn as OnWillChooseFlowReturn
                }
              } else {
                return {}
              }
